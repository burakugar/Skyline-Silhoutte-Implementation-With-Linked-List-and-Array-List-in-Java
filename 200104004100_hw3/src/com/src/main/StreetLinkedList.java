...
public class StreetLinkedList {
    ...
    private int length;
    private int size;
    private LinkedList<Building> buildingArray;
    private LinkedList<Integer> positionList;

    /**
     * no parameter constructor for Street
     */
    public StreetLinkedList() {  /* time complexity O(1) linear time */
        this.length = 0;
        buildingArray = new LinkedList<Building>();
        size = 0;
    }
     ...
     */
    public int getSize() {
        return size; /* time complexity O(1) linear time */
    }
    /**
     *
    ...
    public void setSize(int size) {
        this.size = size;  /* time complexity O(1) linear time */
    }
    public Integer[] append(Integer[] array, int element){
        int len= array.length; // Q(1)
        Integer[] temp =new Integer[len+1]; // Q(1)
        for(int i=0;i<len;i++){  /* Q(n) */
            temp[i]= array[i]; // Q(1)
        }
        temp[len]= element;// Q(1)
        len++;// Q(1)
    ...
    public StreetLinkedList(int length, LinkedList<Building> buildingArray, int size) throws Exception {
        if(size != buildingArray.size()){
            throw new Exception("Street size is different than array size!");
        }
        this.length = length; // Q(1)
        this.buildingArray = buildingArray;// Q(1)
        this.size = size;// Q(1)
        positionList = new LinkedList<Integer>(); // Q(1)
        for (int i = 0; i < buildingArray.size(); i++) { // Q(n)
            for (int j = buildingArray.get(i).getPosition(); j < buildingArray.get(i).getPosition() + buildingArray.get(i).getLength() + 1; j++) { // Q(n)
                positionList.add(j); // Q(1)
            }
        }
    }
    ...
    public LinkedList<Building> getBuildingArray() {
        return buildingArray;// Q(1)
    }
     ...
     */
    public void setBuildingArray(LinkedList<Building> buildingArray) {
        this.buildingArray = buildingArray;// Q(1)
    }
